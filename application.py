...
def get_items():
    return jsonify(json.loads(getItems()))


@application.route('/get-session', methods=['POST'])
def get_session():
...
@application.route('/get-sensors', methods=['POST'])
def get_sensors():
    ...
    for key in attribs.keys():
        ...
        if key in desiredSensors:
            ...
            # return the value with the most recent atime (largest time)
            curr = []
